...
public final int DRINK_POTION = 334;
	...
	{
	if(Server.npcHandler.getNpcListName(NPCID) == null)
	return 399;
		String npc = Server.npcHandler.getNpcListName(NPCID);
		if (npc.contains("bat")) {
			return 1;
		}
		if (npc.contains("cow")) {
	...
	public static int getPlayerBlockSounds(Client c) {
		...
		if (c.playerEquipment[c.playerChest] == 2499 ||
		    ...
		    c.playerEquipment[c.playerChest] == 10330) {//Dragonhide sound
			blockSound = 24;
		}
		    ...
		    c.playerEquipment[c.playerChest] == 11720||//Godwars armour
		    ...
		    c.playerEquipment[c.playerChest] == 1127) {//Metal armour sound
			blockSound = 15;
		} else {
			blockSound = 511;
		}
	...
	public static int GetWeaponSound(Client c)	{
		...
		String wep = c.getItems().getItemName(c.playerEquipment[c.playerWeapon]).toLowerCase();
		...
		if (c.playerEquipment[c.playerWeapon] == 4718 || c.playerEquipment[c.playerWeapon] == 7808)
		{ //Dharok GreatAxe
			return 400;
		}
		...
		if (c.playerEquipment[c.playerWeapon] == 4151) {
			...
			return 401; //Daggers(this is enything that isn't added)
		}
